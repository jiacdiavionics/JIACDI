...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        private Hud3DFpvRenderer hud3DFpvRenderer;
        private volatile bool hud3DFpvEnabled;
        private bool threeScreenModeActive;

        internal HUD ThreeScreenHudControl => hud1;
        internal Control ThreeScreenMapControl => tableMap;
        internal TabPage ThreeScreenQuickTab => tabQuick;
        internal TabPage ThreeScreenGaugesTab => tabGauges;
        internal TabPage ThreeScreenActionsTab => tabActions;
        internal bool IsThreeScreenModeActive => threeScreenModeActive;

        internal void SetThreeScreenModeActive(bool active)
        {
            threeScreenModeActive = active;

            if (active)
            {
                hud1.Visible = true;
                hud1.Enabled = true;
                hud3DFpvRenderer?.Resume();
            }
            else if (!Visible)
            {
                hud3DFpvRenderer?.Pause();
            }
        ...
        public FlightData()
        {
            ...
            */
            ...
            tabControlactions.ItemSize = new Size(0, 36);
            tabControlactions.Padding = new Point(12, 5);
            tabControlactions.ShowToolTips = true;
            foreach (TabPage page in tabControlactions.TabPages)
            {
        ...
        {
            if (!threeScreenModeActive)
            {
                hud3DFpvRenderer?.Pause();
            }

            if (MainV2.MONO && !threeScreenModeActive)
            {
                ...
                try
                {
                        ...
                        tracklast.AddSeconds(2) < DateTime.Now)
                    {
                        ...
                        if (route.Points.Count > 0)
                        {
                            ...
                            if (hasUsablePosition && route.Points.Count == 1 && gMapControl1.Zoom == 3) // 3 is the default load zoom
                            {
                                updateMapPosition(currentloc);
                                updateMapZoom(17);
                            }
        ...
        private async void map3DFpvToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!map3DFpvToolStripMenuItem.Checked)
            {
                StopHud3DFpv();
                return;
            }

            hud3DFpvEnabled = true;
            hudGStreamer.Stop();
            CaptureMJPEG.Stop();
            hud1.bgimage = null;

            var renderer = new Hud3DFpvRenderer(hud1);
            renderer.Failed += Hud3DFpvRenderer_Failed;
            hud3DFpvRenderer = renderer;
            ...
            {
                if (ReferenceEquals(hud3DFpvRenderer, renderer))
                {
                    StopHud3DFpv();
                    map3DFpvToolStripMenuItem.Checked = false;
                }

                if (!(ex is TaskCanceledException) && !(ex is ObjectDisposedException))
                {
                    CustomMessageBox.Show(
                        "Unable to start the 3D Map FPV view:\n\n" + ex.Message,
                        "3D Map FPV");
                }
            }
        }

        private void StopHud3DFpv()
        {
            hud3DFpvEnabled = false;

            Hud3DFpvRenderer renderer = hud3DFpvRenderer;
            hud3DFpvRenderer = null;
            if (renderer != null)
            {
                renderer.Failed -= Hud3DFpvRenderer_Failed;
                renderer.Dispose();
            ...
            if (map3DFpvToolStripMenuItem != null)
            {
                map3DFpvToolStripMenuItem.Checked = false;
            }
        }

        private void Hud3DFpvRenderer_Failed(object sender, EventArgs e)
        {
            Hud3DFpvRenderer renderer = sender as Hud3DFpvRenderer;
            if (renderer == null || !ReferenceEquals(renderer, hud3DFpvRenderer))
            {
                return;
            }

            string error = renderer.LastError;
            StopHud3DFpv();

            CustomMessageBox.Show(
                string.IsNullOrWhiteSpace(error)
                    ? "The 3D Map FPV renderer stopped. Toggle it on to restart."
                    : error,
                "3D Map FPV");
        }
        ...
        void tabControl1_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            using (SolidBrush bgBrush = new SolidBrush(isSelected ? ModernUi.SurfaceRaised : ModernUi.Surface))
            {
            ...
            if (isSelected)
            {
                using (Pen accentPen = new Pen(ModernUi.AccentBright, 2))
                {
                    e.Graphics.DrawLine(accentPen, e.Bounds.Left + 6, e.Bounds.Bottom - 2,
                        e.Bounds.Right - 6, e.Bounds.Bottom - 2);
                }
            }

            using (Pen borderPen = new Pen(ModernUi.Border, 1))
            {
                e.Graphics.DrawLine(borderPen, e.Bounds.Right - 1, e.Bounds.Top + 6,
                    e.Bounds.Right - 1, e.Bounds.Bottom - 6);
            }

            using (Font tabFont = new Font(ModernUi.UiFontFamily, 9F,
                       isSelected ? FontStyle.Bold : FontStyle.Regular, GraphicsUnit.Point))
            {
                TextRenderer.DrawText(e.Graphics, tabPage.Text, tabFont, e.Bounds,
                    isSelected ? ModernUi.TextPrimary : ModernUi.TextSecondary,
        ...
        private void updateBindingSourceWork()
        {
            try
            {
                if ((this.Visible || threeScreenModeActive) && !this.IsDisposed)
                {
                    ...
                    else if (!threeScreenModeActive && tabControlactions.SelectedTab == tabQuick)
                    {
                        MainV2.comPort.MAV.cs.UpdateCurrentSettings(
                            bindingSourceQuickTab.UpdateDataSource(MainV2.comPort.MAV.cs));
                    }
                    else if (!threeScreenModeActive && tabControlactions.SelectedTab == tabGauges)
                    {
                        MainV2.comPort.MAV.cs.UpdateCurrentSettings(
                            bindingSourceGaugesTab.UpdateDataSource(MainV2.comPort.MAV.cs));
                    }
